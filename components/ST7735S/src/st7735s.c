...
void spi_master_init(ST7735_t * dev)
{
	...
	gpio_set_level( GPIO_RESET, 1 ); // 14

	spi_bus_config_t buscfg = {
		.sclk_io_num = GPIO_SCLK, // 13
		.mosi_io_num = GPIO_MOSI, // 15m
		.miso_io_num = -1,
		.quadwp_io_num = -1,
		.quadhd_io_num = -1
	};
...
void lcdInit(ST7735_t * dev, int width, int height)
{
	...
	spi_master_write_data_byte(dev, 0x01); // RTNA = 1, 一行周期
	spi_master_write_data_byte(dev, 0x2C); // FPA = 44
	spi_master_write_data_byte(dev, 0x2D); // BPA = 45
	...
	spi_master_write_data_byte(dev, 0x07); // 111
